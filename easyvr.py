...
# 			return False
	...
	def recognizeWord(self, wordset):
		self.sendCmd(self.CMD_RECOG_SI)
		self.sendArg(wordset)
	...
	def getWord(self):
		if self.recv(5) == self.STS_SIMILAR:
...
while True:
    s = raw_input("")
    sys.stdout.flush()
    client.recognizeWord(wordset)
    i = client.getWord()
    
    # only print valid words
    if i != -1:
	print i
